...
def measure_z(state):
    ...
    if prob_up > random_num:
        return Z_plus, "up"
    elif prob_up + prob_zero > random_num:
        return Z_zero, "zero"
    else:
        return Z_minus, "down"
...
if __name__ == "__main__":
    ...
    axis = input("What axis (x, y, z): ").strip().lower()
    input_type = input("Random input (y/n)? ").strip().lower()
    ...
    print(f"{axis.upper()} zero: {zero_count}")
